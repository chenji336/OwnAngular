...
/*整体的理解
	词法解析器：先解析出token数据
	语法解析器对词法解析器进行解析，把数据解析到AST树上，当然分父子节点
	最后就是对整个树进行compile
*/
...
ASTCompiler.prototype.escape=function(value){
	if(_.isString(value)){
		...
		// null需要标记下，如果没有的话，那么作为第二个数组参数的话会为undefined
		// 就不是null了
		return 'null';
	}
	else{
...
ASTCompiler.prototype.watchFns=function(){
	var result=[];
	var self=this;
	_.forEach(self.state.inputs,function(inputName){
...
function ensureSafeMemberName(name){
	if(name==='constructor'||name==='__proto__'||
		...
		throw 'Attemping to access a disallowed field in Angular expression';
	}
}
...
function ensureSafeObject(obj){
	if(obj){
		if(obj.document && obj.location && obj.alert && obj.setInterval){
			throw 'Referncing window in Angular	 expressions is disallowed!';
		}else if(obj.children &&
			(obj.nodeName || (obj.prop && obj.attr && obj.find))){
			throw 'Referncing DOM nodes in Angular expressions in disallowed!';
