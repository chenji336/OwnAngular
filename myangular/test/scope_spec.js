...
describe('Scope',function(){
	...
	describe('digest',function(){
		...
		it('calls the watcher function with the scope as the argument',function(){
		...
		it("excutes $apply'ed function and starts the digest",function(){
		...
		// $evalAsync与$timeout区别，一个在渲染前执行，一个在DOM渲染后执行
		...
		it("excutes $evalAsync'ed functions added by watch functions",function(){
			scope.aValue=[1,2,3];
			scope.asyncEvaluated=false;
		...
		it("excutes $evalAsync'ed functions even when not dirty",function(){
			scope.aValue=[1,2,3];
		...
		it('has a $$phase field whose value is the current digetst phase',function(){
			scope.aValue=[1,2,3];
		...
		it('schedules a digest in $evalAsync',function(done){
			...
			expect(scope.counter).toBe(0);
			setTimeout(function(){
				// 下面如果加上一个$digest，因为上面已经执行过了，这里数据没有改变，所以不执行
				// scope.$digest();
				expect(scope.counter).toBe(1);
				// 异步的时候需要调用done（），否则屏幕上显示不出来unit的题目
				done();
			},50);
		});
		...
		it("never excutes $applyAsync'ed function in the same cycle",function(done){
			scope.avalue=[1,2,3];
			...
			scope.$digest();
			// 这里就可以看出$applyAsync和$evalAsync的不同了
	...
	describe('$watchGroup',function(){
		...
		it('uses the same array of old and new values on firest run',function(){
			var gotNewValues,gotOldValues;
	...
	describe('inheritance',function(){
		...
		// 当不是对象的时候，子改变，父不改变，只有对象的属性改变的时候，父才改变
		it('chnjiTest',function(){
			var parent=new Scope(),
			child=parent.$new();
			parent.aValue=[1,2];
			...
			child.aValue.push(3);
			expect(parent.aValue).toEqual([1,2,3]);
		});
		...
		it("digests from root on $apply when isolated",function(){
			...
			child2.$apply(function(scope){}); // 会从根节点遍历
			// child2.$digest();  //使用$digest不会从根节点遍历
...
describe('Events',function(){
	...
	it("registers different listeners for every scope",function(){
		...
		scope.$on('someEvent',listener1);
		child.$on('someEvent',listener1);
		isolatedChild.$on('someEvent',listener1);

		expect(scope.$$listeners).toEqual({someEvent:[listener1]});
		expect(child.$$listeners).toEqual({someEvent:[listener1]});
		expect(isolatedChild.$$listeners).toEqual({someEvent:[listener1]});
	});
